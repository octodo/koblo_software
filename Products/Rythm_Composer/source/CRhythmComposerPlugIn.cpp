...
#define PRODUCT_NAME "RhythmComposer"
...
: CBasePlugIn(dynamic_cast<CBaseModule*>(pModule), giAudioMaxBufferSize, dynamic_cast<CBaseDSPEngine*>(new CRhythmComposerDSP()), uiProcessIndex, COMPANY_NAME, PRODUCT_NAME),
	mbUpdateGUISettings(true),
	mbBypass(false)
{
	dynamic_cast<CRhythmComposerDSP*>(mpDSPEngine)->SetChannels(2);
...
{
	CBaseGUI* pBaseGUI = dynamic_cast<CBaseGUI*>(new CRhythmComposerGUI(this, mpParmMan));
...
void CDigitalEchoPlugIn::UpdateEngineData(tint32 iID, tint32 iValue)
{
	...
	if (UpdateEngineSettings()) {
			switch(iID) {
				...
				case giParam_Delay_Time:
					dynamic_cast<CRhythmComposerDSP*>(mpDSPEngine)->Set_Delay_Time(iValue);
					break;
				case giParam_Feedback:
					dynamic_cast<CRhythmComposerDSP*>(mpDSPEngine)->Set_Feedback(iValue );
					break;
				case giParam_HP_Damping:
					dynamic_cast<CRhythmComposerDSP*>(mpDSPEngine)->Set_HF_Damping(iValue );
					break;
				case giParam_LP_Damping:
					dynamic_cast<CRhythmComposerDSP*>(mpDSPEngine)->Set_LF_Damping(iValue );
					break;
				case giParam_Sync:
					dynamic_cast<CRhythmComposerDSP*>(mpDSPEngine)->Set_Sync_To_Tempo((iValue == 0) ? false : true);
					break;
				case giParam_Mix:
					dynamic_cast<CRhythmComposerDSP*>(mpDSPEngine)->Set_Mix(iValue );
					break;
	...
	for (; it != mGUIs.end(); it++) {
		CBaseGUI* pGUI = *it;
		dynamic_cast<CRhythmComposerGUI*>(pGUI)->UpdateEngineData(0, 0, iID, iValue);
	}
}
...
void CDigitalEchoPlugIn::UpdateAllGUI()
{
	...
	for (; it != mGUIs.end(); it++) {
		CBaseGUI* pGUI = *it;
		dynamic_cast<CRhythmComposerGUI*>(pGUI)->UpdateAll();
	}
}
