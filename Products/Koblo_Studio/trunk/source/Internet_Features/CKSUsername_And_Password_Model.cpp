...
CKSUsername_And_Password_Model::CKSUsername_And_Password_Model(CKSPlugIn* pKSPlugIn):
...
void CKSUsername_And_Password_Model::Open_Username_And_Password_Dialog()
{
	...
	tbool bWindow_Is_Open = (mpKSPlugIn->GetGlobalParm(giSign_In_Window, giSectionGUI) != 0);
	if(!bWindow_Is_Open){
...
void CKSUsername_And_Password_Model::On_Username_And_Password_OK()
{
	...
	if(mpKSPlugIn->mbUpload_Project && mpKSPlugIn->Create_Project_Handshake()){
		mpKSPlugIn->mbUpload_Project = false;
		mpKSPlugIn->Upload_Project();
	}
