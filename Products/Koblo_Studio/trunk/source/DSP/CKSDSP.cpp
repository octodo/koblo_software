...
CKSDSP::CKSDSP(CKSPlugIn* pPlugIn)
...
/*	tint32 iStream;
	...
	mppBusses = new CChannel*[giNumber_Of_Busses];
	tint32 iBus;
	for (iBus = 0; iBus < giNumber_Of_Busses; iBus++) {
		mppBusses[iBus] = new CChannel(this, iBus, true, mppAUXes);
		mppBusses[iBus]->Initialize();
	}

	mpMaster = new CChannel(this, 1024, true, mppAUXes);
	mpMaster->Initialize();
...
void CKSDSP::Destroy()
{
	delete dynamic_cast<CKSDSP*>(this);
}
...
void CKSDSP::SetMaxBufferSize(tint32 uiMaxBufferSize)
{
	CBaseDSPEngine::SetMaxBufferSize(uiMaxBufferSize);
}

void CKSDSP::SetSampleRate(tuint32 uiSampleRate)
{
...
void CKSDSP::SetChannels(tuint32 uiChannels)
{
	muiChannels = uiChannels;
}

void CKSDSP::SetInputsForChannel(tuint32 uiChannel, tint32 iInCount, tint32 iInFirst)
{
	if ((tuint32)(iInFirst+iInCount) > muiChannels)
		return;
	
	CChannel* pChannel = mppChannels[uiChannel];
	pChannel->SetInputChannelCount(iInCount);
	pChannel->SetFirstInput(iInFirst);
}

void CKSDSP::SetModeForChannel(tuint32 uiChannel, tint32 iMode)
{
	// Lasse, ooops!!...
	/*
	...
	*/
	...
	// .. Lasse
	CChannel* pChannel = mppChannels[uiChannel];
	pChannel->SetChannelMode(iMode);
}

void CKSDSP::SetPanningLeftRightForChannel(tuint32 uiChannel, tfloat32 fLeftRight)
{
	// Lasse, ooops!!...
	/*
	if (uiChannel >= muiChannels)
...
void CKSDSP::SetPanningFrontBackForChannel(tuint32 uiChannel, tfloat32 fFrontBack)
{
	// Lasse, ooops!!...
	/*
	if (uiChannel >= muiChannels)
...
void CKSDSP::LimitDataFromStreams(tbool bEnableLimiting, tint64 iFirstSampleIx, tint64 iFinalSampleIx)
{
	...
	for (tint32 iStem = 0; iStem < giNumber_Of_Tracks; iStem++) {
		...
		CChannel* pStem = mppChannels[iStem];
		pStem->LimitDataFromStream(bLimit_ThisStem, iFirstSampleIx, iFinalSampleIx);
	}
} // LimitDataFromStreams


void CKSDSP::ProcessMono(float** /*ppfOut*/, const float** /*ppfIn*/, long /*lC*/)
{
	// This is never called
}

void CKSDSP::ProcessStereo(float** ppfOut, const float** ppfIn, long lC)
{
...
//					*mpBufferMix += *pBuffer;
				...
				if (!mpPlugIn->IsInProgressTaskState()) {
					...
					CChannel* pTrack = mppChannels[iTrack];
...
void CKSDSP::Process(tfloat** ppfSamplesOut, const tfloat** ppfSamplesIn, tuint32 iNrOfSamples)
{
	if (muiChannels == 1) {
...
void CKSDSP::GetInput(CBuffer* mpBuffer, tint32 iChannels, tint32 iInputChannel, tint32 iNrOfSamples)
{
	if (iChannels == 1) {
...
//void CKSDSP::SetTrackInputFile(tint32 iTrack, const std::string& sTargetPathName)
//{
//	CWaveFile* pWaveFile = new CWaveFile();
...
void CKSDSP::SetSongPosition(tuint64 uiPosNew)
{
 	muiSongPos	=	uiPosNew;
/*	tint32 iStream;
	for (iStream = 0; iStream < giNrOfStreams; iStream++) {
...
void CKSDSP::SetSongPosition_AndResetEffectsTails(tuint64 uiPosNew)
{
 	muiSongPos	=	uiPosNew;
/*	tint32 iStream;
	for (iStream = 0; iStream < giNrOfStreams; iStream++) {
...
tint32 CKSDSP::GetStutterCounter()
{
...
tuint32 CKSDSP::GetFinalSoundPos()
{
...
/*
void CKSDSP::UpdateAUXData(tint32 iID, tint32 iValue, tint32 iAUX)
{
...
//			mppAUXes[iAUX]->SetInputChannelCount(iValue);
		...
		case giParam_ChMode:
			...
			//mppAUXes[iAUX]->SetInputChannelCount(iValue);
			// .. Lasse
			mppAUXes[iAUX]->SetChannelMode(iValue);
			break;
...
*/
void CKSDSP::UpdateBussData(tint32 iID, tint32 iValue, tint32 iBus)
{
	switch(iID) {
		case giParam_ChVol:
//			if(!mppChannels[iBus]->mbMuteFlag)
		...
		case giParam_ChMode:
			...
			//mppBusses[iBus]->SetInputChannelCount(iValue);
			// .. Lasse
			mppBusses[iBus]->SetChannelMode(iValue);
			break;
...
void CKSDSP::UpdateMasterData(tint32 iID, tint32 iValue)
{
	switch(iID) {
...
void CKSDSP::UpdateChannelData(tint32 iID, tint32 iValue, tint32 iChannel)
{
...
*/
	switch(iID) {
		...
		case giParam_ChMode:
			mppChannels[iChannel]->SetChannelMode(iValue);
			break;
...
void CKSDSP::UpdateAUX1Data(tint32 iID, tint32 iValue)
{
...
void CKSDSP::UpdateAUX2Data(tint32 iID, tint32 iValue)
{
...
tint32 CKSDSP::FindDestinationNumberOfChannels(tint32 iDestination)
{
...
tint32 CKSDSP::GetNumberOfInputChannels(tint32 iChannelOrBus)
{
	tint32 iChannels = 0;
...
tint32 CKSDSP::GetNumberOfChannelsForPanner(tint32 iChannelOrBus)
{
	tint32 iChannels = 0;
	tint32 iBusNr = iChannelOrBus - giNumber_Of_Tracks;
	if (iBusNr < 0)
...
tint32 CKSDSP::GetDestinationForChannelOrBus(tint32 iChOrBus)
{
...
void CKSDSP::SetDestinationForChannelOrBus(tint32 iChOrBus, tint32 iDestination, tint32 iDestNumberOfChannels)
{
	tint32 iBusNr = iChOrBus - giNumber_Of_Tracks;
	if (iBusNr < 0) {
		// This is a channel		
...
tbool CKSDSP::UpdateDestinationForChannelOrBus_ButOnlyIfItMatches(tint32 iChOrBus, tint32 iDestination, tint32 iDestNumberOfChannels)
{
	tint32 iBusNr = iChOrBus - giNumber_Of_Tracks;
	if (iBusNr < 0) {
		// This is a channel
...
CKSDSP::RegionSearchHandle CKSDSP::GetFirstRegion(SRegionInfo& rInfo)
{
	tint32 iChannel;
	for (iChannel = 0; iChannel < giNumber_Of_Tracks; iChannel++) {
		CChannel* pChannel = mppChannels[iChannel];

		std::list<CChannel::SChannelRegionInfo*>::const_iterator it = pChannel->GetRegionList().begin();
		if (it != pChannel->GetRegionList().end()) {
			// Track not empty, return first region

			rInfo.uiRegionID = (*it)->pSoundObject->GetID();
			rInfo.uiChannel = iChannel;
...
void CKSDSP::GetNextRegion(CKSDSP::RegionSearchHandle& Handle, CKSDSP::SRegionInfo& rInfo)
{
	SRegionSearchInfo* pInfo = (SRegionSearchInfo*)Handle;

	tuint32 uiChannelOrg = pInfo->uiChannelCur;

	tuint32 uiChannel;
	for (uiChannel = uiChannelOrg; uiChannel < giNumber_Of_Tracks; uiChannel++) {
		CChannel* pChannel = mppChannels[uiChannel];

		std::list<CChannel::SChannelRegionInfo*>::const_iterator it;
		if (uiChannel == uiChannelOrg) {
			it = pInfo->itCur;
			it++;
		}
		else {
		...
		if (it != pChannel->GetRegionList().end()) {
			...
			rInfo.uiRegionID = (*it)->pSoundObject->GetID();
			rInfo.uiChannel = uiChannel;
			rInfo.uiStartPos = (*it)->uiTrackPosStart;
			rInfo.uiEndPos = rInfo.uiStartPos + (*it)->pSoundObject->GetDuration() - 1;

			pInfo->uiChannelCur = uiChannel;
			pInfo->itCur = it;
...
void CKSDSP::FinishRegionSearch(CKSDSP::RegionSearchHandle Handle)
{
...
void CKSDSP::Refresh_Region_GUI(tint32 iRegion, tuint32 iTrack)
{	
...
tint32 CKSDSP::CreateRegion(const std::string& sSoundListItemName, 
							tint32 iChannel, 
							tuint64 uiTrack_Pos, 
...
void CKSDSP::Set_Session_Size(tuint64 uiRegionEndSample)
{
...
void CKSDSP::Sanitize_Session_End_Sample()
{
	tuint64 uiEndPos = 0;
	SRegionInfo Info;
...
tbool CKSDSP::CalcTrackDuration(tint32 iTrack, tint64* piFirstSample, tint64* piFinalSample)
{
...
void CKSDSP::MoveRegion(tuint32 uiRegionID, tint32 iChannelNew, tuint64 uiTrackPosStartNew)
{
	SRegionInfo RegionInfo;
	GetRegionInfo( RegionInfo, uiRegionID);
	
	CSoundObject* pRegionSoundObject	=	mppChannels[RegionInfo.uiChannel]->GetRegionSoundObject(uiRegionID);
...
void  CKSDSP::GetRegionInfo(SRegionInfo& RegionInfo, tuint32 uiRegionID)
{
...
tuint64 CKSDSP::GetRegionSize(const std::string& sSoundPathName,tuint64 uiSamplePosStart, tint64 uiSamplePosEnd)
{
	tint64 iSize = mppChannels[0]->GetRegionSize(0, sSoundPathName, uiSamplePosStart, uiSamplePosEnd);
	return iSize;
}

tuint64 CKSDSP::GetRegionSize(tuint32 uiID )
{
	SRegionInfo RegionInfo;
	GetRegionInfo( RegionInfo, uiID);
	CSoundObject* pRegionSoundObject	=	mppChannels[RegionInfo.uiChannel]->GetRegionSoundObject(uiID);
	return	pRegionSoundObject->GetDuration();
}

SRegion_Drawing_Info CKSDSP::Get_Region_Drawing_Info(tuint iRegionID)
{
	...
	CSoundObject* pSoundObject	=	mppChannels[RegionInfo.uiChannel]->GetRegionSoundObject(iRegionID);
	GetRegionInfo( RegionInfo, iRegionID);
...
void CKSDSP::GetRegionPeakFile(tuint32 uiID, IFile** ppFile, tint32 iChannel, tint32 iSize)
{
	SRegionInfo RegionInfo;
	GetRegionInfo(RegionInfo, uiID);

	CSoundObject* pSoundObject = mppChannels[RegionInfo.uiChannel]->GetRegionSoundObject(uiID);
	if (pSoundObject)
...
void CKSDSP::DeleteAllRegionsForTrack(tint32 iTrack)
{
...
void CKSDSP::DeleteRegion(tuint32 uiTrack, tuint32 uiRegionID)
{
	SRegionInfo RegionInfo;
	GetRegionInfo( RegionInfo, uiRegionID);
	// Delete region on channel
	mppChannels[RegionInfo.uiChannel]->DeleteRegion(uiRegionID);
	// Find new end of song
	Sanitize_Session_End_Sample();
	// Delete region on GUI
	DeleteRegionOnGUI(RegionInfo.uiChannel, uiRegionID);

}

void CKSDSP::DeleteRegionOnGUI(tuint32 uiTrack, tuint32 uiRegionID)
{
...
tbool CKSDSP::SaveTrackRegionDataToChunk(tint32 iTrack, IChunk* pChunk)
{
	CChannel* pChannel = GetChannel(iTrack);
	std::list<CChannel::SChannelRegionInfo*>::const_iterator it = pChannel->GetRegionList().begin();
	std::string sRc = "";
	...
	if (iSaveVersion == 1) {
		...
		for (; it != pChannel->GetRegionList().end(); it++) {
			...
			CSoundObject* pObject = pRegionInfo->pSoundObject;
...
tbool CKSDSP::CreateRegionFromChunkData(tint32 iTrack, IChunk* pChunk)
{
...
void CKSDSP::DeleteTrack(tuint32 uiTrack)
{
...
tbool CKSDSP::IsAudioDeviceDuplex()
{
	return IsAudioDeviceDuplex(-1, -1, -1, -1);
} // IsAudioDeviceDuplex


tbool CKSDSP::IsAudioDeviceDuplex(tint32 iInMgrIx, tint32 iInDevIx, tint32 iOutMgrIx, tint32 iOutDevIx)
{
...
tbool CKSDSP::OpenAudioDevice_FromPrefs_Output(tbool* pbAudioDevice_LastOpenCrashed, tbool* pbMidiDevice_LastOpenCrashed)
{
	LoadPrefs();
...
tbool CKSDSP::OpenAudioDevice_FromPrefs_Input(tbool* pbAudioDevice_LastOpenCrashed)
{
	LoadPrefs();
...
tbool CKSDSP::OpenAudioDevice_Output(tint32 iMgrIx, tint32 iDevIx, tchar* pszDevName, tint32 iFreqHz, tint32 iBuffSize, tbool bInvokeStart)
{
...
tbool CKSDSP::OpenAudioDevice_Input(tint32 iMgrIx, tint32 iDevIx, tchar* pszDevName, tint32 iBuffSize, tbool bInvokeStart)
{
	// First of all see if we should be running as duplex
...
tbool CKSDSP::OpenAudioDevice_Duplex(tint32 iMgrIx, tint32 iDevIx, tchar* pszDevName, tint32 iFreqHz, tint32 iBuffSize, tbool bInvokeStart)
{
...
void CKSDSP::StartAudioDevice_Duplex()
{
	if (mpAudio_Duplex)
...
void CKSDSP::StartAudioDevice_Output()
{
	if (mpAudio_Output)
...
void CKSDSP::StartAudioDevice_Input()
{
	if (mpAudio_Input)
		mpAudio_Input->Start();
...
void CKSDSP::SetTrackSelection(tint32 iTrack, tint64 uiSelection_Pos, tint64 fDuration, tint32 uiSelection_Type, tint32 iRegionID)
{
...
void CKSDSP::Deselect_All_Tracks()
{
...
void CKSDSP::Deselect_Track(tuint32 uiTrack)
{
...
STrackSelectionInfo CKSDSP::SelectRegion(tint32 iRegionID)
{ 
	...
	CSoundObject* pRegionSoundObject	=	mppChannels[RegionInfo.uiChannel]->GetRegionSoundObject(uiID);
	//std::string sClipName				=	pRegionSoundObject->GetSoundListItemName();
	tint32 iChannel						=	RegionInfo.uiChannel;
...
void CKSDSP::CutRegion(tuint32 uiTrack, tuint32 uiRegionID, tuint64 uiCutPos)
{ 
	...
	CSoundObject* pRegionSoundObject	=	mppChannels[RegionInfo.uiChannel]->GetRegionSoundObject(uiRegionID);
	std::string sClipName				=	pRegionSoundObject->GetSoundListItemName();
	tfloat64 uiRegionTrackPosStart		=	mppChannels[RegionInfo.uiChannel]->GetRegionPosOnTrack(uiRegionID);	
...
//	tuint64 uiFadeInLength				=	pRegionSoundObject->GetFadeInLength();
	...
	mppChannels[RegionInfo.uiChannel]->TrimRegion(uiRegionID, uiRegionTrackPosStart, uiSoundPos, uiSoundPos + uiCutPos-1);
...
void CKSDSP::TrimRegion(tuint32 uiTrack, tuint32 uiRegionID, tbool bStart, tint64 iSamplePos)
{ 
	...
	CSoundObject* pRegionSoundObject	=	mppChannels[RegionInfo.uiChannel]->GetRegionSoundObject(uiRegionID);
	std::string sClipName				=	pRegionSoundObject->GetSoundListItemName();
	tint64 iRegionTrackPosStart			=	mppChannels[RegionInfo.uiChannel]->GetRegionPosOnTrack(uiRegionID);	
...
//	tuint64 uiFadeOutLength				=	pRegionSoundObject->GetFadeOutLength();
	...
	//		iSoundPos = iSampleEnd - 32;
		...
		mppChannels[RegionInfo.uiChannel]->TrimRegion(uiRegionID, iRegionTrackPosStart, iSoundPos, iSampleEnd);
		mpRegionCallback->Refresh_Region_GUI(uiRegionID, uiTrack);
	}
	else{
...
void CKSDSP::NormaliseRegion()
{ 
	
	
}

void CKSDSP::InverseRegion()
{ 
	
	
}

const tchar* CKSDSP::GetClipNameOfSelectedRegion()
{
	for(tint32 uiTrack=0; uiTrack<giNumber_Of_Tracks; uiTrack++){
		if(mpTrackSelectionInfo[uiTrack].uiSelection_Type = giSelect_Region &&  mpTrackSelectionInfo[uiTrack].iRegionID != -1)
		{
...
tuint64 CKSDSP::Fade_In(tuint32 uiRegionID, tuint64 uiFadeInLength)
{ 
...
tuint64 CKSDSP::Fade_Out(tuint32 uiRegionID, tuint64 uiFadeOutLength)
{ 
...
void CKSDSP::Region_Volume(tuint32 uiRegionID, tfloat32 fRegion_Volume)
{ 
...
void CKSDSP::CloseAudioDevice_Output()
{
	if (mpAudio_Output) {
		mpAudio_Output->Destroy();
		mpAudio_Output = NULL;
...
void CKSDSP::CloseAudioDevice_Input()
{
	if (mpAudio_Input) {
		mpAudio_Input->Destroy();
		mpAudio_Input = NULL;
...
void CKSDSP::CloseAudioDevice_Duplex()
{
	if (mpAudio_Duplex) {
		mpAudio_Duplex->Destroy();
		mpAudio_Duplex = NULL;
...
tint32 CKSDSP::GetAudioDeviceMenuItems(ge::IPopupMenu::SMenuItemList& rlist, tint32& riSelectedValue)
{
	rlist.iItemCount = 0;
...
tint32 CKSDSP::GetAudioDeviceMenuItemsInput(ge::IPopupMenu::SMenuItemList& rlist, tint32& riSelectedValue)
{
	rlist.iItemCount = 0;
...
tint32 CKSDSP::GetAudioFreqsMenuItems(ge::IPopupMenu::SMenuItemList& rlist, tint32& riSelectedValue)
{
	rlist.iItemCount = 0;
...
tint32 CKSDSP::GetAudioBuffsMenuItems(ge::IPopupMenu::SMenuItemList& rlist, tint32& riSelectedValue)
{
	rlist.iItemCount = 0;
...
tbool CKSDSP::OpenAudioDevice_FromMenu_Output(tint32 iListValue, tint32 iFreqHz, tint32 iBuffSize)
{
	tbool bSuccess = false;
...
tbool CKSDSP::OpenAudioDevice_FromMenu_Input(tint32 iListValue, /*tint32 iFreqHz,*/ tint32 iBuffSize)
{
	tbool bSuccess = false;
...
void CKSDSP::ClearAllMeters()
{
	mpsMeters_All->Zero();
} // ClearAllMeters


void CKSDSP::GetAllMeters_MaybeClear(SMeters_All* psMeters_All, tbool bClear)
{
...
void CKSDSP::SetMeters_Master(tfloat32* pfAbsValues)
{
...
void CKSDSP::SetMeters_Track(tint32 iTrack, tfloat32* pfAbsValues)
{
	// Set all (max 7.1 surround) channels for track
...
void CKSDSP::SetMeters_Bus(tint32 iBus, tfloat32* pfAbsValues)
{
...
void CKSDSP::SetMeters_AUX(tint32 iAUX, tfloat32* pfAbsValues)
{
...
void CKSDSP::Loop_On(tbool bLoop)
{
	mbLoop_On	= bLoop;
	Update_Loop();
...
void CKSDSP::Set_Loop_Start(tuint64 uiStart)
{
...
void CKSDSP::Set_Loop_End(tuint64 uiEnd)
{
...
void CKSDSP::Set_Loop_To_Selection()
{
...
tint32 CKSDSP::GetNrOfInputChannels()
{
	if (mpAudio_Input) {
