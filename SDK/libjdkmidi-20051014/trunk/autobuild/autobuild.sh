...
options_dir="${relative_dir}/autobuild-options"
...
show_targets() {
    for i in $(find "${options_dir}/targets/" -maxdepth 1 -type f -print); do
        echo "    " $(basename "$i")
    done
}

show_platforms() {
    for i in $(find "${options_dir}/platforms/" -maxdepth 1 -type f -print); do
        echo "    " $(basename "$i")
    done
}

show_options() {
    for i in $(find "${options_dir}/options/" -maxdepth 1 -type f -print); do
        echo "    " $(basename "$i")
    done
}

show_packages() {
    for i in $(find "${options_dir}/packages/" -maxdepth 1 -type f -print); do
        echo "    " $(basename "$i")
    done
}
...
target_path="${options_dir}/targets/${target}"
platform_path="${options_dir}/platforms/${platform}"
option_path="${options_dir}/options/${option}"
package_path="${options_dir}/packages/${package}"

echo \"${options_dir}/autobuild.sh\" \"$target\" \"$platform\" \"$option\" \"$package\" "\\" >./.autobuild.sh
for i in "$@"; do
    echo  " \"$i\" \\" >> ./.autobuild.sh
done
echo " \"\$@\" " >> ./.autobuild.sh
