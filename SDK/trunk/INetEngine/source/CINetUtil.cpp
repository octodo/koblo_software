...
tbool CINetUtil::GetValue(tchar* SourceString, tint32* OutputLength, tchar** OutputBuffer) {
	*OutputBuffer = NULL;
	*OutputLength = 0;
...
tbool CINetUtil::FindLowerCaseString(tchar* pszLowerCaseString, tchar* pszBuffer, tint32 iBufferLen, tchar** ppszIndexAt, tchar** ppszIndexAfter)
{
...
tbool CINetUtil::GetTagContentsPtr(const tchar* pszTagNameConst, tchar* pszSourceString, tint32 iSourceLen, tchar** ppszOutput, tint32* piOutputLength)
{
	*ppszOutput = NULL;
	*piOutputLength = 0;
...
tbool CINetUtil::GetValuePtr(tchar* szSourceString, tchar** ppszOutput, tint32* piOutputLength) {
...
void CINetUtil::ReleaseBuffer(tchar** Buffer) {
    if (*Buffer == NULL) return;
	delete *Buffer;
	*Buffer = NULL;
}

bool CINetUtil::IsResultOK(tchar* SourceString) {
    return strstr((const char*) SourceString,"<result status=\"ok\">") != NULL;
}

tfloat64 CINetUtil::GetNumericWebValue(tchar* Parameters, tchar* FileName) throw(IException*)
{
	int length;
	tchar* szS;
	try {
		GetWebFile(Parameters,FileName,&length,&szS);
	}
	catch (IException* ex) {
		ReleaseBuffer(&szS);
        throw(ex);
...
bool CINetUtil::GetNextElement(tchar **S, tchar *Name, int NameLength, bool *IsFolder, tint32 *Price)
{
	//...<element name="xxx" isFolder="True/False" price="ppp"/>...
	try {
    if (strlen((const char *) *S) == 0) return false;
	char *nameStr = new char[strlen((const char *) *S)+1];
    char *isFolderStr = new char[strlen((const char *) *S)+1];
    char *priceStr = new char[strlen((const char *) *S)+1];
	nameStr[0] = 0;
	isFolderStr[0] = 0;
	priceStr[0] = 0;
    if (!GetMiddleStr(S, "<element name=\"", "\" ", nameStr)) return false;
    if (!GetMiddleStr(S, "isFolder=\"", "\" ", isFolderStr)) return false;
    if (!GetMiddleStr(S, "price=\"", "\"/>", priceStr)) return false;
	if (strlen(nameStr)+1 > (unsigned int)NameLength) return false;
	strcpy((char*) Name, nameStr);
	*IsFolder = strcmp(isFolderStr,"True") == 0;
	*Price = atoi(priceStr);
	}
	catch (...) {
	  return false;
...
bool CINetUtil::GetMiddleStr(tchar **s, const char *sStart, const char *sEnd, char *sMiddle)
{
	tchar *pStart;
	tchar *pEnd;
