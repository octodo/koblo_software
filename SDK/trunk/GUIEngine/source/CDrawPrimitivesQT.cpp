...
CDrawPrimitivesQT::CDrawPrimitivesQT()
	: mpWnd(NULL)
{
}
...
{
	return dynamic_cast<IDrawPrimitives*>(new CDrawPrimitivesQT());
}

void CDrawPrimitivesQT::Destroy()
{
	delete dynamic_cast<CDrawPrimitivesQT*>(this);
}

void CDrawPrimitivesQT::Init(IWindow* pWnd)
{
	mpWnd = pWnd;
}

void CDrawPrimitivesQT::DrawPoint(const SRect& RectUpdate, const SPos& Pos, const SRGB& Colour)
{
...
void CDrawPrimitivesQT::DrawLine(const SRect& RectUpdate, const SPos& PosFrom, const SPos& PosTo, const SRGB& Colour)
{
	// Get buffer to draw into
	tuchar* pDestBuffer;
	mpWnd->GetDrawPointer((void**)&pDestBuffer);
	...
	if (abs(cx) > abs(cy)) {
		...
		for (; x <= PosEnd.iX && x < SizeDest.iCX && x < RectUpdate.iX + RectUpdate.iCX; x++) {
//			if (x < RectUpdate.iX) {
//				continue;
//			}
		...
		for (; y <= PosEnd.iY && y < SizeDest.iCY && y < RectUpdate.iY + RectUpdate.iCY; y++) {
//			if (y < RectUpdate.iY) {
//				continue;
//			}
...
void CDrawPrimitivesQT::DrawRect(const SRect& RectUpdate, const SRect& RectFill, const SRGB& Colour)
{
	// Get buffer to draw into
	tuchar* pDestBuffer;
	mpWnd->GetDrawPointer((void**)&pDestBuffer);
...
void CDrawPrimitivesQT::InvertRect(const SRect& RectUpdate, const SRect& RectInvert)
{
	// Get buffer to draw into
	tuchar* pDestBuffer;
	mpWnd->GetDrawPointer((void**)&pDestBuffer);
