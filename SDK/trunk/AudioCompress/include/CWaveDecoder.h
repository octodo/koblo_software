...
#ifndef _ac_cwave_decoder_h
#define _ac_cwave_decoder_h

//#include <lame.h>


class CWaveDecoder : public virtual IDecoder, public virtual CDecoder
{
public:
	CWaveDecoder();
	virtual ~CWaveDecoder();
	...
	//! Prepare for dummy decoding of a set of wave streams into one or two WAVE streams (mono, stereo or dual mono)
	/*!
	...
	*/
	...
	//! Dummy decode an wave stream and append to the previously initialized output Wave stream(s)
	/*!
		\param pfInputStream [in]: Source stream (wave format) already opened with read permission
...
protected:
	//tint32 miIn_BitWidth;
	tint32 miIn_Channels;
	tint32 miIn_BlockAlign;
	tint32 miIn_SampleRate;
	tint32 miIn_Samples;

};

#endif // #ifndef _ac_cwave_decoder_h